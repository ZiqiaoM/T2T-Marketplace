...
const Home: NextPage = () => {
  ...
  useEffect(()=>{
    ...
    if (!userInfo) {
      // If user information is not found, redirect to login page
      router.push('/login');
    } else {
      // If user information is found, use email to query database for corresponding user information
      getUserInfo(userInfo.email).then((userInfoFromDb) => {
        // Merge the user information from session/local storage and the database
        const mergedUserInfo = { ...userInfo, ...userInfoFromDb };
      ...
      <Container>
        <Row>
          <Col lg='12' className='text-center'>
            ...
            <p className='mb-1 mt-4 feature_text'>Don&apos;t leave your belongings behind rather pass them on.</p>
          </Col>
        </Row>
      </Container>
    </section>
        ...
        <Row>
          ...
          <Col lg='12'>
            <div className="pro_category d-flex align-items-center justify-content-center gap-5">
              ...
              <button className={`all_btn  ${Category==='ALL'?'proBtnActive': ''}`} onClick={()=>setCategory('ALL')}>All</button>
              <button className={`d-flex align-items-center gap-2 ${Category==='KITCHENWARES'?'proBtnActive': ''}`} onClick={()=>setCategory('KITCHENWARES')}>Kitchenwares</button>
              <button className={`d-flex align-items-center gap-2 ${Category==='CLOTHING'?'proBtnActive': ''}`} onClick={()=>setCategory('CLOTHING')}>Clothing</button>
              <button className={`d-flex align-items-center gap-2 ${Category==='ELECTRONICS'?'proBtnActive': ''}`} onClick={()=>setCategory('ELECTRONICS')}>Electronics</button>
              <button className={`d-flex align-items-center gap-2 ${Category==='FURNITURES'?'proBtnActive': ''}`} onClick={()=>setCategory('FURNITURES')}>Furnitures</button>
              <button className={`d-flex align-items-center gap-2 ${Category==='OTHERS'?'proBtnActive': ''}`} onClick={()=>setCategory('OTHERS')}>Others</button>             
